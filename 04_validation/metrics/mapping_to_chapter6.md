...
| 階段 | 公式 | 說明 |
|------|------|------|
| **L1：偵測延遲** | `t_detected - t_trigger` | IoT 事件 → 系統偵測 |
| **L2：語意推理延遲** | `t_task_created - t_detected` | Traversal / TIAA 推理時間 |
| **L3：派發延遲** | `t_action_start - t_task_created` | Workflow / API 任務派發 |
| **L4：執行延遲** | `t_action_end - t_action_start` | 實際行動執行 |
